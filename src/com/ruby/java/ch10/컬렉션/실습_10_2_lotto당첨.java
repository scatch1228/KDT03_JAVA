...
public class 실습_10_2_lotto당첨 {
	...
	// 구매한 Lotto 화면에 출력 여부
	private static final boolean PRINTLOTTO = false; //<<설정하세요~

	// lottWin과 lotto를 비교해서 당첨여부를 확인
	// 맞춘 번호 개수를 리턴하면 된다.
	private static int hitTest(Lotto1 lottoWin, Lotto1 lotto) {
		//convert lotto to list
		List<Integer> list = new ArrayList<>(lotto.getLotto());
		//count the number matched
		int count = 0;
		for (Integer lot : list) {
			if (lottoWin.getLotto().contains(lot)) {
				count++;
			}
		}
	...
	public static void main(String[] args) {
		...
		if (!PRINTLOTTO)	System.out.println("당첨번호=" + lottoWin.toString());
