...
class SeasonalDiscount implements Discountable {
	...
	public double getDiscountedPrice(double price) {
		return price*(1-seasonalDiscountRate);
	}
...
class Order3 extends SeasonalDiscount {
	...
	// 생성자
	public Order3(Customer3 customer, double d) {
		super(d);
	...
	private double calculateTotal() {
		...
		for (int i=0; i<count; i++) {
			x+= items[i].getPrice()*quantities[i] ;
		}
		return x;
	}
	...
	public void printOrderSummary() {
		...
		//제품정보
		for (int i=0; i<count; i++) {
			System.out.println("제품명:" + items[i].getName() + ", 단가:" 
								+ items[i].getPrice() + ", 수량:" 
								+ quantities[i] + ", 가격:" 
		...
		System.out.println("시즌할인가격:" + getDiscountedPrice(customer.applyDiscount(calculateTotal())));
		}
		System.out.println("=============================================");
	}
...
*/
		...
		// 배열에 전자제품과 의류패션 객체 추가
				...
				double seasonalDiscountRate = 0.05;
